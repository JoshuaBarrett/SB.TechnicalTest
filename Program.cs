...
namespace SB.TechnicalTest
{
    class Program
    {
        ...
        static int Attempt2()
        {
            int dropTarget = Building.NumberFloors / 2;
            int highestSuccessDrop = 0;
            int lowestfailDrop = Building.NumberFloors + 1;

            while (true)
            {
                if (Building.DropMarble(dropTarget))
                {
                    highestSuccessDrop = dropTarget;
                }
                else
                {
                    lowestfailDrop = dropTarget;                  
                }

                if (Math.Abs(lowestfailDrop - highestSuccessDrop) == 1) // A difference of 1 means we have reached the highest successful floor
                {
                    return highestSuccessDrop;        
                }
                else
                {
                    dropTarget = (highestSuccessDrop + lowestfailDrop) / 2; // Dont care about fractions so happy to divide by pure ints :)
                }
            }
        }
    }
